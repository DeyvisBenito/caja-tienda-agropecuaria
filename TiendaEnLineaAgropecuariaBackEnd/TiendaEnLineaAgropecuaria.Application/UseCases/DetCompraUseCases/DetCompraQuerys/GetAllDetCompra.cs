...
{
    public class GetAllDetCompra
    {
        private readonly IRepositorioDetCompra repositorioDetCompra;

        public GetAllDetCompra(IRepositorioDetCompra repositorioDetCompra)
        {
            this.repositorioDetCompra = repositorioDetCompra;
        }

        public async Task<IEnumerable<DetalleCompraDTO>> ExecuteAsync()
        {
            var detallesCompra = await repositorioDetCompra.Get();
