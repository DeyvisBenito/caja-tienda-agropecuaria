...
{
    public class GetAllDetVentaByVentaId
    {
        private readonly IRepositorioDetVenta repositorioDetVenta;

        public GetAllDetVentaByVentaId(IRepositorioDetVenta repositorioDetVenta)
        {
            this.repositorioDetVenta = repositorioDetVenta;
        }

        public async Task<IEnumerable<DetalleVentaDTO>> ExecuteAsync(int ventaId, int sucursalId)
        {
            var detallesVenta = await repositorioDetVenta.GetByVentaId(ventaId, sucursalId);
